...
|---------------|----------|
| `getServerSession(authOptions)` throws or rejects | Confirmed: No `try`/`catch` or error boundary around this call is present in `src/app/page.tsx`; behavior on failure is not evidenced in the reviewed code. |
