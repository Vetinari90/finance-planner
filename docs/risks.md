...
Server-side re-validation of the item amount is directly confirmed in the planned-items route handler `src/app/api/plans/[planId]/items/route.ts`, which defines `CreateItemSchema = z.object({ ..., amountCents: z.number().int().min(0), ... })` and rejects any request body that fails `CreateItemSchema.safeParse(body)` with a 400 response before creating the `plannedItem` record. This confirms server-side re-validation of the amount is present, independent of the module's narrative documentation.
The server-side re-validation claim is confirmed, not unconfirmed: `src/app/api/plans/[planId]/items/route.ts` implements a Zod `CreateItemSchema` requiring `amountCents: z.number().int().min(0)`, and returns a 400 error (`{ error: "Invalid input", details: ... }`) when `CreateItemSchema.safeParse(body)` fails, before persisting the item via `prisma.plannedItem.create`. This is directly verifiable from the planned-items module's own route handler.
